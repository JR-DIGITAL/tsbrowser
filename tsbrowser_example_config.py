...
path = "./samples/samples.geojson"

# set an output directory to store flag files
# if None, write to the vector file directory
flag_dir = "./flags"
...
attr_id = "ID"
attr_q_loc = "raster_location"  # location of quality raster files for the feature
attr_i_loc = "quality_location"  # location of image raster files for the feature

# set quality raster file search options
q_pattern = "L3A*score*v1.tif"
q_recursive = False

# set image raster file search options
i_pattern = "L3A_????????_m2_v1.tif"
i_recursive = False

# set options for retrieving the image acquisition timestamp
t_mode = "filename"  # possible values: metadata, filename
t_slice = slice(4, 12)  # used if t_mode == filename
t_format = "%Y%m%d"  # used if t_mode == filename
